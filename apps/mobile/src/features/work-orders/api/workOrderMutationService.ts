...
export async function createMaintenanceWorkOrder(input: { equipmentId: string; reason: string; priority: string }) {
  ...
  const { data, error } = await supabase.rpc('rpc_create_maintenance_work_order', {
    ...
    p_source_type: 'MANUAL_SCAN',
    p_source_id: input.equipmentId.trim(),
    p_reason: reason,
