...
export function NotificationsScreen({ onBack }: { onBack: () => void }) {
  ...
  async function load(refresh = false) {
    ...
    try { setItems(await listNotifications(unreadOnly)); setError('') }
    catch (reason) { setError(reason instanceof Error ? reason.message : 'Không tải được thông báo.') }
  ...
  async function open(item: NotificationItem) {
    if (!item.readAt) {
      try { await markNotificationRead(item.notificationId); setItems((current) => current.map((row) => row.notificationId === item.notificationId ? { ...row, readAt: new Date().toISOString() } : row)) } catch { /* inbox remains usable */ }
    }
  }
