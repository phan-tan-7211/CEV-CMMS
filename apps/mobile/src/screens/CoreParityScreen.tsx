...
import { ActivityIndicator, Alert, Image, Pressable, ScrollView, StyleSheet, Text, TextInput, View, type GestureResponderEvent, type LayoutChangeEvent } from 'react-native'
...
export type CoreParityTab = 'checklists' | 'fields' | 'downtime' | 'floor'
...
export function CoreParityScreen({ onBack, initialTab = 'checklists', onOpenEquipment }: { onBack: () => void; initialTab?: CoreParityTab; onOpenEquipment?: (equipmentId: string) => void }) {
  const [tab, setTab] = useState<CoreParityTab>(initialTab)
  useEffect(() => { setTab(initialTab) }, [initialTab])
  return <SafeAreaView style={styles.safe}><StatusBar style="dark"/><View style={styles.header}><Pressable onPress={onBack} style={styles.iconButton}><Ionicons name="chevron-back" size={26} color="#101828"/></Pressable><View style={styles.headerCopy}><Text style={styles.headerTitle}>Core CMMS nâng cao</Text><Text style={styles.headerSub}>Checklist · Custom Fields · Downtime · Floor Plan</Text></View></View><PickerRow items={TABS.map((item) => ({ id: item.key, label: item.label }))} selected={tab} onSelect={(key) => setTab(key as CoreParityTab)}/>{tab === 'checklists' ? <ChecklistsPanel/> : null}{tab === 'fields' ? <CustomFieldsPanel/> : null}{tab === 'downtime' ? <DowntimePanel/> : null}{tab === 'floor' ? <FloorPlanPanel onOpenEquipment={onOpenEquipment}/> : null}</SafeAreaView>
}
...
function FloorPlanPanel({ onOpenEquipment }: { onOpenEquipment?: (equipmentId: string) => void }) {
  ...
  function onPlanLayout(event: LayoutChangeEvent){const {width,height}=event.nativeEvent.layout;setPlanSize({width,height})}
  function onPlanPress(event: GestureResponderEvent){if(!planSize.width||!planSize.height)return;const xr=Math.min(1,Math.max(0,event.nativeEvent.locationX/planSize.width));const yr=Math.min(1,Math.max(0,event.nativeEvent.locationY/planSize.height));setX(String(Math.round(xr*1000)/10));setY(String(Math.round(yr*1000)/10))}
