    ...
    <SafeAreaView style={styles.safeArea} edges={['top', 'bottom']}>
      ...
      {loading && items.length === 0 ? <View style={styles.center}><ActivityIndicator size="large" color="#155EEF" /></View> : (
        <FlatList
          ...
          renderItem={({ item }) => (
            <Pressable onPress={() => onOpenWorkOrder(item.workOrderId)} style={({ pressed }) => [styles.card, pressed && styles.pressed]}>
              ...
              <View style={styles.badges}><View style={styles.statusBadge}><Text style={styles.statusText}>{statusLabel(item.status)}</Text></View>{item.priority ? <View style={styles.priorityBadge}><Text style={styles.priorityText}>{item.priority}</Text></View> : null}</View>
          ...
          ListEmptyComponent={<View style={styles.center}><Ionicons name="checkmark-done-circle-outline" size={42} color="#98A2B3" /><Text style={styles.emptyTitle}>Không có Work Order</Text><Text style={styles.emptyText}>Không có dữ liệu phù hợp với nhóm dashboard này.</Text></View>}
        />
      )}
    </SafeAreaView>
  )
...
const styles = StyleSheet.create({
  ...
  card: { padding: 16, borderRadius: 18, borderWidth: StyleSheet.hairlineWidth, borderColor: '#E1E1EA', backgroundColor: '#FFFFFF' },
  ...
  badges: { marginTop: 10, flexDirection: 'row', gap: 7 },
