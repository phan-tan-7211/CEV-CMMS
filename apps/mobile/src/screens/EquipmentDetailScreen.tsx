...
export function EquipmentDetailScreen({ equipmentId, onBack, onOpenStatus }: { equipmentId: string; onBack: () => void; onOpenStatus: (currentStatus: string) => void }) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeEquipmentDetail(equipmentId, (nextItem) => {
      if (!active) return
      setItem(nextItem); setLoading(false); setError('')
    })
    void getEquipmentDetailSnapshot(equipmentId).then((snapshot) => {
      if (!active || !snapshot) return
      hasSnapshot = true; setItem(snapshot); setLoading(false)
    }).finally(() => {
      void revalidateEquipmentDetail(equipmentId).then((fresh) => {
        if (!active) return
        setItem(fresh); setLoading(false); setError('')
      }).catch((loadError) => {
        if (!active) return
