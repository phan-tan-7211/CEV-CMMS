...
test('preventive maintenance opens plans first and can switch to work orders', async ({ page }) => {
  ...
  await expect(workspace.getByRole('heading', { level: 1, name: 'Bảo trì phòng ngừa' })).toBeVisible()
...
test('scheduler surfaces PM state, overlap lanes and month overflow list', async ({ page }) => {
  ...
  await detail.getByLabel('Đóng', { exact: true }).click()
  await scheduler.getByRole('button', { name: /^Nguồn lực/ }).click()
  ...
  await scheduler.getByRole('button', { name: /^Lịch/ }).click()
